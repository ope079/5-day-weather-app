    ...
    $.ajax({url : countryQueryUrl}).then(function(response){
        ...
        $.ajax({url : queryUrl}).then(function(weatherResponse){
                ...
                for(let i = 0; i < weatherList.length; i += 8){
                    ...
                    let weatherForecastEL = $(` <h1> ${weatherResponse.city.name} (${moment(weatherForecast.dt_txt, "YYYY-MM-DD HHHH").format("DD/MM/YYYY")}) <img src="${`http://openweathermap.org/img/w/${weatherForecast.weather[0].icon}.png`}"  alt="Weather icon"></h1>
